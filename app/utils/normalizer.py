...
def find_duplicates(
    products: List[Dict[str, Any]], threshold: float = 0.85
) -> List[List[int]]:
    """
...
def compare_products(products: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for i, product in enumerate(products):
        aligned = {
            ...
            "is_best_rating": product.get("rating")
            == best.get("rating", {}).get("value"),
        }

        # Calculate discount percentage
        if aligned["original_price"] and aligned["price"]:
            discount = (
                (aligned["original_price"] - aligned["price"])
                / aligned["original_price"]
            ) * 100
    ...
    return {"products": aligned_products, "best": best, "count": len(products)}
